...
module.exports = class Jrhexex extends LeavingCreator {
    mul() {
        ...
        var final = JrhexexCells0.concat(JrhexexCells1, JrhexexCells2, JrhexexCells3, JrhexexCells4, JrhexexCells5)
        let newCell = final[Math.floor(Math.random() * final.length)]
        for(i in newCell){
            if (newCell) {
                var newX = newCell[i];
                var newY = newCell[i];
